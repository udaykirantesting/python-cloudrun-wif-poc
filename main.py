...
# Configuration - Ensure these environment variables are set in your deployment
# For Cloud SQL, the DB_URL format is usually: 
# postgresql+psycopg2://<user>:<pass>@<public_ip>/voting_db
...
class Vote(db.Model):
    ...
    city_1 = db.Column(db.String(50), nullable=False)
    city_2 = db.Column(db.String(50), nullable=False)
    city_3 = db.Column(db.String(50), nullable=False)
...
def index():
    if request.method == 'POST':
        ...
        if len(selected_cities) == 3:
            new_vote = Vote(
                city_1=selected_cities[0],
                city_2=selected_cities[1],
                city_3=selected_cities[2]
            )
